import React, { useEffect, useState } from 'react';
import Modal from '../../_design/components/alerts/Modal';
import { API, Auth } from 'aws-amplify';
import Loader from '../../_design/components/core/Loader';
import Text from '../../_design/components/core/Text';
import Input from '../../_design/components/form/Input';
import Button from '../../_design/components/core/Button';
import { Organisation } from '../../../../functions/src/types/organisation';

interface CreateStudentModalProps {
  isOpen: boolean;
  onClose: () => void;
  organisationId: string;
}

const CreateStudentModal: React.FC<CreateStudentModalProps> = ({ isOpen, onClose, organisationId }) => {
    const [organisation, setOrganisation] = useState<Organisation | null>(null);
    const [saveLoading, setSaveLoading] = useState(false);

    const handleSave = async () => {
        setSaveLoading(true);

        try {
            await Auth.signUp({
                username: (document.getElementsByName("username")[0] as HTMLInputElement).value,
                password: (document.getElementsByName("password")[0] as HTMLInputElement).value,
                attributes: {
                    given_name: (document.getElementsByName("givenName")[0] as HTMLInputElement).value,
                    family_name: (document.getElementsByName("familyName")[0] as HTMLInputElement).value,
                    "custom:initial": organisationId,
                    "custom:role": "STUDENT"
                },
            });
            onClose();
        } finally {
            setSaveLoading(false);
        }
    };

    useEffect(() => {
        async function fetchPack() {
            setOrganisation(null);
            const organisation: Organisation = await API.get("api", `/organisation/${organisationId}`, {});
            setOrganisation(organisation);
        }
        if (isOpen) {
            fetchPack();
        }
    }
    , [isOpen, organisationId]);

    return (
        <Modal isOpen={isOpen} onClose={onClose} closeOnBackdropClick>
        <Modal.Header>
            <Text variant="h3" noMargin>Create student in {organisation && organisation.name}</Text>
        </Modal.Header>
        <Modal.Body>
            {
                organisation ? (
                    <div className="flex flex-col gap-2">
                        <Input name="givenName" label="Given name" />
                        <Input name="familyName" label="Family name" />
                        <Input name="username" label="Username" />
                        <Input name="password" label="Password" type="password" />
                    </div>
                ) : (
                    <Loader size={20} />
                )
            }
        </Modal.Body>
        <Modal.Footer>
            <Button onClick={onClose} colorScheme="secondary">Cancel</Button>
            <Button onClick={handleSave} colorScheme="success" loading={saveLoading} disabled={!organisation}>Create</Button>
        </Modal.Footer>
        </Modal>
    );
};

export default CreateStudentModal;
